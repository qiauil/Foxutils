...
class EmaGradClip(Callback):
    ...
    def on_before_optimizer_step(self):
        if self.grad_norm_emas==0.0:
            total_norm, clipped = self._clip_grad_norm_(self.trainer.model.parameters(), 
                                                         max_norm=10000,
                                                         clip_norm=1,)
        else:
            total_norm, clipped = self._clip_grad_norm_(
...
class ConstantGradClip(Callback):
    ...
    def on_before_optimizer_step(self):
        clip_grad_norm_(self.trainer.model.parameters(), 
